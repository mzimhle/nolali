        ...
        <div class="col-sm-9">
          <div class="portlet">
            ...
            <div class="portlet-content">
              <form id="validate-basic" action="/booking/payment.php?code={$bookingData.booking_code}" method="POST" data-validate="parsley" class="form parsley-form" enctype="multipart/form-data">			
				...
				<table class="table table-bordered">	
					...
					{foreach from=$paymentData item=item}
					  <tr>	
						<td valign="top">R {$item._payment_amount|number_format:0:".":","}</td>
						<td valign="top">{$item._payment_paid_date}</td>
						<td valign="top">{if $item._payment_file eq ''}N / A{else}<a href="http://{$item.campaign_domain}{$item._payment_file}" target="_blank">Download</a>{/if}</td>					
						<td valign="top">
							<button value="Delete" class="btn btn-danger" onclick="deleteModal('{$item._payment_code}', '{$item.booking_code}', 'payment'); return false;">Delete</button>
						</td>		
					  </tr>			     
					{foreachelse}
						<tr>
                ...
                <div class="form-group">
					...
					{if isset($errorArray._payment_amount)}<br /><span class="error">{$errorArray._payment_amount}</span>{/if}					  
                </div>
                <div class="form-group">
					...
					{if isset($errorArray._payment_paid_date)}<br /><span class="error">{$errorArray._payment_paid_date}</span>{/if}					  
                </div>
                <div class="form-group">
