...
if(isset($_GET['action']) && trim($_GET['action']) == 'tablesearch') {
	...
	if($invoiceData) {
		for($i = 0; $i < count($invoiceData['records']); $i++) {
			...
			$all[$i] = array( 
					$item['invoice_added'],
					'<a href="/catalogue/invoice/details.php?code='.$item['invoice_code'].'" title="'.$item['invoice_notes'].'" alt="'.$item['invoice_notes'].'">REF#'.$item['invoice_code'].'</a>',
					...
					($item['invoice_pdf'] == '' ? $item['invoice_make'] : '<a href="http://'.$zfsession->domainData['campaign_domain'].$item['invoice_pdf'].'" target="_blank">'.$item['invoice_make'].'</a>'),
...
$smarty->display('catalogue/invoice/default.tpl');
