...
if(count($_POST) > 0) {
	...
	if(isset($_POST['_price_code']) && trim($_POST['_price_code']) == '') {
		$errorArray['_price_code'] = 'Please select product and price';
		$formValid = false;		
	} else {
		$priceData = $priceObject->getByCode(trim($_POST['_price_code']), 'PRODUCTITEM');
		
		if(!$priceData) {
			$errorArray['_price_code'] = 'Selected product price not valid.';
			$formValid = false;				
		}
	}
	...
	if(count($errorArray) == 0 && $formValid == true) {
		...
		$data = array();
		$data['invoiceitem_name'] 			= $priceData['product_name'];
		$data['invoiceitem_quantity'] 		= trim($_POST['invoiceitem_quantity']);
		$data['invoiceitem_description'] 	= $priceData['productitem_name'];
		$data['invoiceitem_amount'] 		= $priceData['_price_cost'];
		$data['invoice_code']				= $invoiceData['invoice_code'];

		$success	= $invoiceitemObject->insert($data);			
		
		if($success) {
			header('Location: /catalogue/invoice/item.php?code='.$invoiceData['invoice_code']);
			exit;
		}
	}
...
$smarty->display('catalogue/invoice/item.tpl');
