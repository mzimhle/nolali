...
if(!is_dir(realpath(__DIR__.'/../../../../').$zfsession->domainData['campaign_directory'].'/media/uploaded/')) { 
	mkdir(realpath(__DIR__.'/../../../../').$zfsession->domainData['campaign_directory'].'/media/uploaded/', 0777, true);
}
define('NICUPLOAD_PATH', realpath(__DIR__.'/../../../../').$zfsession->domainData['campaign_directory'].'/media/uploaded/'); 

// Set the path (relative or absolute) to
// the directory to save image files
                                      
define('NICUPLOAD_URI', 'http://'.$zfsession->domainData['campaign_domain'].'/media/uploaded');   // Set the URL (relative or absolute) to
...
if($_SERVER['REQUEST_METHOD']=='POST') { // Upload is complete
    ...
    /* $filename = $id.'.'.$ext; */
	$filename = md5($id.rand(100000,100000000).time()).'.'.$ext;
