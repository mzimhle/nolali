			...
			if($linkContent) {
				...
				if($classbox != '') {					
					...
					if(!preg_match('/^([a-zA-Z0-9])+([a-zA-Z0-9\._-])*@([a-zA-Z0-9_-])+([a-zA-Z0-9\._-]+)+$/', trim($emailDetails))) {
						...
						$data['spam_fax'] 		= isset($faxmatches[1][0]) && trim($faxmatches[1][0]) != '' ? trim($faxmatches[1][0]) : null;					
						/******************************************************************************************/							

						/* FAX *************************************************************************************/																				
						$regexp = '/<span>Share Call Fax: <\/span>(.*?)<\/div>/'; $faxmatches = null;
						preg_match_all($regexp, $contactDetails, $faxmatches);
						$data['spam_fax'] 		= isset($faxmatches[1][0]) && trim($faxmatches[1][0]) != '' ? trim($faxmatches[1][0]) : $data['spam_fax'];					
						/******************************************************************************************/	

						/* FAX *************************************************************************************/																				
						$regexp = '/<span>Cell: <\/span>(.*?)<\/div>/';
						preg_match_all($regexp, $contactDetails, $cellmatches);
						$data['spam_cell'] 		= isset($cellmatches[1][0]) && trim($cellmatches[1][0]) != '' ? trim($cellmatches[1][0]) : null;					
						/******************************************************************************************/	

						/* LINK *************************************************************************************/																				
						$linkDetails = str_get_html($contactDetails)->find('.web a', 0)->innertext;				
						$data['spam_link']	= $linkDetails;
						/*******************************************************************************************/
