...
$fileObject 					= new File(array('pdf', 'xls', 'txt', 'docx', 'doc', 'cvs'));
...
if(count($_POST) > 0) {
	...
	if(isset($_FILES['itemfile'])) { 
		...
		if((int)$_FILES['itemfile']['size'] == 0) {
			...
			$ext = array_search($_FILES['itemfile']['type'], $fileObject->mime_types); 
			
			if($ext != '') {
				if(!$fileObject->valideExt($ext)) { 
					$errorArray['itemfile'] = 'Invalid file type';
					$formValid = false;						
				}
			} else {
				$errorArray['itemfile'] = 'Invalid file type';
	...
	if(count($errorArray) == 0 && $formValid == true) {
		...
		if(file_put_contents($file, file_get_contents($_FILES['itemfile']['tmp_name']))) {
			...
			header('Location: /admin/documents/item.php?code='.$code);
			exit;
		} else {
			$errorArray['campaigndocumentitem_name'] = 'required';
			$formValid = false;			
 ...
 */	
$smarty->display('adminclient/MU3H/documents/item.tpl');
