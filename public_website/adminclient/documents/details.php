...
if(count($_POST) > 0) {
	...
	if(count($errorArray) == 0 && $formValid == true) {
		...
		if(isset($campaigndocumentData)) {
			...
			header('Location: /admin/documents/item.php?code='.$campaigndocumentData['campaigndocument_code']);
			exit;					
			...
			header('Location: /admin/documents/item.php?code='.$data['campaigndocument_code']);
			exit;
		}					
	}
...
$smarty->display('adminclient/MU3H/documents/details.tpl');
?>