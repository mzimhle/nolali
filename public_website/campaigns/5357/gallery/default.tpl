		...
		$(document).ready(function() {
			...
			$(".fancybox_{/literal}{$item.gallery_code}{literal}_click").click(function() {
				$.fancybox.open([
					...
					{ href : {/literal}'http://{$campaign.campaign_domain}/{$image.galleryimage_path}/big_{$image.galleryimage_code}{$image.galleryimage_extension}', title : '' {literal}}{/literal}{if $smarty.foreach.fancyimage.last}{else},{/if}{literal}
		...
		<div id="gallery">
			...
			<div class="galleryitem">
				<a href="javascript:;" class="fancybox_{$item.gallery_code}_click"> 
					<img class="extra-img png" src="/{$item.galleryimage_path}/tmb_{$item.galleryimage_code}{$item.galleryimage_extension}" title="{$campaign.campaign_name} - {$item.gallery_name}" alt="{$item.campaign_name} - {$item.gallery_name}" width="200" />
				</a>
				<br />
