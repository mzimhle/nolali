...
<head>
	...
	<title>{$campaign.campaign_name} - Contact Us</title>
    ...
    <script type="text/javascript">
		function initialize() {
		  ...
		  var marker = new google.maps.Marker({
			  position: myLatlng,
			  map: map,
			  title: '{/literal}{$campaign.campaign_name}{literal}'
		  });
		}
...
<div id="wrap">
	...
	<div id="main">
		<h2>Contact {$campaign.campaign_name}!</h2>		
		...
		<div id="contact-area">			
			<form method="post" action="/contact/" name="detailsForm" id="detailsForm">
				<table>
					<tr>
						<td class="left">
							...
							{if isset($errorArray.inquiry_name)}<em class="error">{$errorArray.inquiry_name}</em>{/if}
						</td>
					...
					<tr>
						<td class="left">
							...
							{if isset($errorArray.inquiry_email)}<em class="error">{$errorArray.inquiry_email}</em>{/if}
						</td>
					...
					<tr>
						<td class="left">
							...
							{if isset($errorArray.inquiry_number)}<br /><em class="error">{$errorArray.inquiry_number}</em>{else}<em>(Optional e.g. 0735698741){/if}
						</td>
					...
					<tr>
						<td class="left">
							...
							{if isset($errorArray.inquiry_subject)}<em class="error">{$errorArray.inquiry_subject}</em>{/if}
						</td>
						<td>
					...
					<tr>
						<td class="left" valign="top">
							...
							{if isset($errorArray.inquiry_comments)}<em class="error">{$errorArray.inquiry_comments}</em>{/if}
						</td>
				...
				{if isset($errorArray.inquiry_captcha)}<em class="error">Incorrect captcha details, please fill in the correct characters in the box.</em>{/if}
				<br /><br />
