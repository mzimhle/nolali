...
class class_price extends Zend_Db_Table_Abstract
{
   //declare table variables
	...
	public function insert(array $data) {
		...
		if((isset($data['_price_reference']) && trim($data['_price_reference']) != '') && 
				(isset($data['_price_product']) && trim($data['_price_product']) != '') &&
					(isset($data['_price_number']) && trim($data['_price_number']) != '')) {
			
			$priceData = $this->getByProductPrice($data['_price_product'], $data['_price_reference'], $data['_price_number']);
     ...
     * @return object
	 */
	public function getByProductPrice($product, $code, $number) {
		...
		$select = $this->_db->select()	
							...
							->where('_price._price_number = ?', $number)
							->where('_price._price_active = 1')
							->limit(1);
	...
	public function pairs($product) {

		switch($product) {
			case 'PRODUCTITEM' :
				$select = $this->select()
				   ->from(array('_price' => '_price'), array('_price._price_code', "concat(productitem.productitem_name, ' ( R ',_price._price_cost,')', ' - ', _price_number, ' item(s)') as _price_name"))
     ...
     * @return object
	 */
	public function getByProduct($product, $code, $number = '') {
     ...
     * @return object
	 */
	public function getByActive($product, $code) {
