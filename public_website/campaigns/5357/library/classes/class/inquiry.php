   ...
   //declare table variables
	...
	{	
		...
		if($search == '') {
			$select = $this->_db->select()	
						->from(array('inquiry' => 'inquiry'))	
						->joinLeft(array('campaign' => 'campaign'), 'campaign.campaign_code = inquiry.campaign_code', array())	
						->joinLeft(array('areapost' => 'areapost'), 'areapost.areapost_code = inquiry.areapost_code', array('areapost_name'))	
						->where('inquiry_deleted = 0 and campaign_deleted  = 0')
						->where('inquiry_active = 1 and campaign_active = 1')				
						->where($this->_campaign->_campaignsql)		
						->order('inquiry_added');
			...
			$select = $this->_db->select()	
						...
						->where('inquiry_active = 1 and campaign_active = 1')				
						->where($this->_campaign->_campaignsql)	
						->where("concat(inquiry_name, 	inquiry_email, inquiry_number) like lower(?)", "%$search%")				
						->order("LOCATE('$search', inquiry.inquiry_name)");					
