...
if(isset($_GET['id']) && trim($_GET['id']) != '') {
	...
	header('Location: /content/announcement/');
	exit;
}
	...
	if(count($errorArray) == 0) {
		if(isset($_FILES['mediafiles']) && count($_FILES['mediafiles']['name']) > 0) {
			for($i = 0; $i < count($_FILES['mediafiles']['name']); $i++) {
				...
				$directory	= $zfsession->config['path'].'/media/content/announcement/'.$contentData['content_id'].'/'.$data['media_code'];
				...
				$data['media_path']	= '/media/content/announcement/'.$contentData['content_id'].'/'.$data['media_code'].'/';
				$data['media_ext']		= '.'.$ext ;
		...
		header('Location: /content/announcement/media.php?id='.$contentData['content_id']);
		exit;
	}
...
$smarty->display('content/announcement/media.tpl');
?>