...
if(isset($_GET['action']) && trim($_GET['action']) == 'search') {
	...
	$filter[] = array('filter_type' => 'ANNOUNCEMENT');	
	...
	if($contentData) {
		for($i = 0; $i < count($contentData['records']); $i++) {
			...
			$contents[$i] = array(
                ($item['media_code'] != '' ? '<img src="'.$zfsession->config['site'].$item['media_path'].'tny_'.$item['media_code'].$item['media_ext'].'"/>' : '<img src="/images/no-image.png" width="220"/>'),
				'<a href="/content/announcement/details.php?id='.trim($item['content_id']).'">'.trim($item['content_name']).'</a>',
                trim($item['content_date_start']),
                trim($item['content_date_end']),
...
$smarty->display('content/announcement/default.tpl');
?>