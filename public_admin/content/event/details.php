...
if(count($_POST) > 0) {
	...
	if(!isset($_POST['content_time_start'])) {
		$errors[] = 'Please add start time of the content';	
	} else if(trim($_POST['content_time_start']) == '') {
		$errors[] = 'Please add start time of the content';		
    } else if($contentObject->validateTime(trim($_POST['content_time_start'])) == '') {
        $errors[] = 'Please add a valid time';
    }

	if(!isset($_POST['content_time_end'])) {
		$errors[] = 'Please add end time of the content';	
	} else if(trim($_POST['content_time_end']) == '') {
		$errors[] = 'Please add end date of the content';	
    } else if($contentObject->validateTime(trim($_POST['content_time_end'])) == '') {
        $errors[] = 'Please add a valid time';
    }
	...
	if(count($errors) == 0) {
		...
		$data['content_map_latitude']   = trim($_POST['content_map_latitude']);
		$data['content_map_longitude']  = trim($_POST['content_map_longitude']);        
        $data['content_type']	        = 'EVENT';		
        $data['content_text']	        = trim($_POST['content_text']);		
        $data['content_date_start']	    = trim($_POST['content_date_start']).' '.trim($_POST['content_time_start']).':00';	
        $data['content_date_end']	    = trim($_POST['content_date_end']).' '.trim($_POST['content_time_end']).':00';	        
		/* Insert or update. */
	...
	if(count($errors) != 0) {
		...
		header('Location: /content/event/media.php?id='.$success);
		exit;
	}
}

$smarty->display('content/event/details.tpl');
?>