	...
	if(count($errorArray) == 0) {
		if(isset($_FILES['mediafiles']) && count($_FILES['mediafiles']['name']) > 0) {
			for($i = 0; $i < count($_FILES['mediafiles']['name']); $i++) {
				...
				$directory	= $zfsession->config['path'].'/media/content/event/'.$contentData['content_id'].'/'.$data['media_code'];
				...
				$data['media_path']	= '/media/content/event/'.$contentData['content_id'].'/'.$data['media_code'].'/';
				$data['media_ext']		= '.'.$ext ;
		...
		header('Location: /content/event/media.php?id='.$contentData['content_id']);
		exit;
	}
...
$smarty->display('content/event/media.tpl');
?>