...
if(isset($_GET['action']) && trim($_GET['action']) == 'search') {
	...
	$filter[] = array('filter_type' => 'GALLERY');	
	...
	if($contentData) {
		for($i = 0; $i < count($contentData['records']); $i++) {
			...
			$contents[$i] = array(
				...
				'<a href="/content/gallery/details.php?id='.trim($item['content_id']).'">'.trim($item['content_name']).'</a>',
...
$smarty->display('content/gallery/default.tpl');
?>