...
if(count($_POST) > 0) {
	...
	if(!isset($_POST['content_url'])) {
		$errors[] = 'Please add the url of the news';	
	} else if(trim($_POST['content_url']) == '') {
		$errors[] = 'Please add the url of the news';	
	}
	...
	if(count($errors) == 0) {
        ...
        $data['content_url']	= trim($_POST['content_url']);		      
		/* Insert or update. */
	...
	if(count($errors) != 0) {
		...
		header('Location: /content/news/media.php?id='.$success);
		exit;
	}
}

$smarty->display('content/news/details.tpl');
?>