	...
	if(count($errorArray) == 0) {
		if(isset($_FILES['mediafiles']) && count($_FILES['mediafiles']['name']) > 0) {
			for($i = 0; $i < count($_FILES['mediafiles']['name']); $i++) {
				...
				$directory	= $zfsession->config['path'].'/media/content/news/'.$contentData['content_id'].'/'.$data['media_code'];
				...
				$data['media_path']	= '/media/content/news/'.$contentData['content_id'].'/'.$data['media_code'].'/';
				$data['media_ext']		= '.'.$ext ;
		...
		header('Location: /content/news/media.php?id='.$contentData['content_id']);
		exit;
	}
...
$smarty->display('content/news/media.tpl');
?>