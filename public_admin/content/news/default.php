	...
	if($contentData) {
		for($i = 0; $i < count($contentData['records']); $i++) {
			...
			$contents[$i] = array(
				...
				'<a href="/content/news/details.php?id='.trim($item['content_id']).'">'.trim($item['content_name']).'</a> - <a href="'.trim($item['content_url']).'" target="_blank">View</a>',
...
$smarty->display('content/news/default.tpl');
?>