...
if(isset($_GET['getproducts'])) {
	...
	if($productData) {
		...
		foreach($productData as $item) {
			$html .= "<option value='".$item['product_id']."'> ".$item['product_name']." - R ".$item['price_amount']." </option>";
		}
	} else {
...
if(isset($_GET['addproduct'])) {
	...
	if(!isset($_GET['modal_product_id'])) {
		...
		if(!$productData) {
			...
			if(!isset($_GET['modal_product_quantity'])) {
				...
				$errors[] = 'Please add the quantity';	
			} else if((int)trim($_GET['modal_product_quantity']) > (int)$productData['product_left'] && $productData['product_type'] != 'SERVICE') {
	...
	if(count($errors)  == 0) {
		...
		if(!$success) {
			...
			$errorArray['result']	= 1;	
            $invoiceObject->updateInvoice($invoiceData['invoice_id']);
		}
	}
