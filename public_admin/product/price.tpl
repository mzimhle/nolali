      ...
      <div class="container">
        ...
        <div class="section-wrapper">
          ...
          <div class="row">
			...
			<form action="/product/price.php?id={$productData.product_id}" method="POST">
				<table class="table table-bordered">	
					...
					{foreach from=$priceData item=item}
						<tr>
							...
							<td {if $item.price_active eq '1'}class="success"{else}class="error"{/if}>{if $item.price_active eq '1'}<button class="btn" onclick="deleteModal('{$item.price_id}', '{$productData.product_id}', 'price'); return false;">Deactivate</button>{else}N / A{/if}</td>
						</tr>			     					
					{/foreach}
					{else}
						<tr>
