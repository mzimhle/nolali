...
if(isset($_GET['id']) && trim($_GET['id']) != '') {
	...
	$productData = $contentObject->getById($id);
...
if(isset($_GET['status_id'])) {
	...
	if($mediaData) {
		$mediaObject->updatePrimary('CONTENT', $productData['product_id'], $mediaData['media_id'], $mediaData['media_category']);	
	} else {
	...
	if(count($errorArray) == 0) {
		if(isset($_FILES['mediafiles']) && count($_FILES['mediafiles']['name']) > 0) {
			for($i = 0; $i < count($_FILES['mediafiles']['name']); $i++) {
				...
				$data['media_item_id']      = $productData['product_id'];
				...
				$directory	= $zfsession->config['path'].'/media/product/'.$productData['product_id'].'/'.$data['media_code'];
				...
				$data['media_path']	= '/media/product/'.$productData['product_id'].'/'.$data['media_code'].'/';
				$data['media_ext']		= '.'.$ext ;
				/* Check for other medias. */
				$primary = $mediaObject->getPrimary('CONTENT', $productData['product_id'], 'IMAGE');		
				if($primary) {
		...
		header('Location: /product/media.php?id='.$productData['product_id']);
		exit;
	}
...
$smarty->display('product/media.tpl');
?>