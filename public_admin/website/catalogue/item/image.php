...
if(count($_FILES) > 0) {
	...
	if(count($errorArray) == 0 && $formValid == true) {
		...
		for($i = 0; $i < count($_FILES['imagefiles']['name']); $i++) {
			if(isset($_FILES['imagefiles']['size'][$i])) {
				if((int)$_FILES['imagefiles']['size'][$i] != 0 && trim($_FILES['imagefiles']['name'][$i]) != '') {
					...
					$directory					= realpath(__DIR__.'/../../../../').$zfsession->domainData['campaign_directory'].'/media/productitems/'.$productitemData['productitem_code'].'/'.$data['productitemimage_code'];
					...
					$data['productitemimage_path']			= 'media/productitems/'.$productitemData['productitem_code'].'/'.$data['productitemimage_code'];
		...
		if(is_numeric($success)) {
			header('Location: /website/catalogue/item/image.php?code='.$productitemData['productitem_code']);
			exit;
		}
	}
...
$smarty->display('website/catalogue/item/image.tpl');
