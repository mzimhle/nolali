...
<div id="container">
  ...
  <div id="content">
	...
	<div class="inner">     
    <h2>Manage <span class="success">{$domainData.campaign_name}</span> Items</h2>		
			  ...
			  <tr>
				<td>
					{if $item.productitemimage_path neq ''}
						<img src="http://{$item.campaign_domain}/{$item.productitemimage_path}/tny_{$item.productitemimage_code}{$item.productitemimage_extension}"  width="70" />
					{else}
				...
				<td align="left"><a href="/website/catalogue/item/details.php?code={$item.productitem_code}">{$item.productitem_name}</a></td>	
				<td align="left"><button onclick="deleteitem('{$item.productitem_code}'); return false;">Delete</button></td>				
			  </tr>
			  {/foreach}     
			  </tbody>
			</table>
