...
if(count($_FILES) > 0) {
	...
	if(count($errorArray) == 0 && $formValid == true) {
		...
		for($i = 0; $i < count($_FILES['imagefiles']['name']); $i++) {
			if(isset($_FILES['imagefiles']['size'][$i])) {
				if((int)$_FILES['imagefiles']['size'][$i] != 0 && trim($_FILES['imagefiles']['name'][$i]) != '') {
					...
					$directory					= realpath(__DIR__.'/../../../../').$zfsession->domainData['campaign_directory'].'/media/products/'.$productData['product_code'].'/'.$data['productimage_code'];
					...
					$data['productimage_path']			= 'media/products/'.$productData['product_code'].'/'.$data['productimage_code'];
		...
		if(is_numeric($success)) {
			header('Location: /website/catalogue/product/image.php?code='.$productData['product_code']);
			exit;
		}
	}
...
$smarty->display('website/catalogue/product/image.tpl');
