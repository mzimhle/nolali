		...
		for($i = 0; $i < count($_FILES['imagefiles']['name']); $i++) {
			if(isset($_FILES['imagefiles']['size'][$i])) {
				if((int)$_FILES['imagefiles']['size'][$i] != 0 && trim($_FILES['imagefiles']['name'][$i]) != '') {
					...
					$directory					= realpath($_SERVER['DOCUMENT_ROOT'].'/../').$zfsession->domainData['campaign_directory'].'/media/gallery/view/'.$galleryData['gallery_code'].'/'.$data['galleryimage_code'];
					...
					$data['galleryimage_path']			= 'media/gallery/view/'.$galleryData['gallery_code'].'/'.$data['galleryimage_code'];
...
$smarty->display('website/gallery/view/image.tpl');
